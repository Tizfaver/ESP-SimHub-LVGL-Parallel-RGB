...
public:
	...
	static void touch_read(lv_indev_drv_t * indev_driver, lv_indev_data_t * data) {
		SHCustomProtocol *self = static_cast<SHCustomProtocol *>(indev_driver->user_data);
		...
		if (pos[0] > 0 && pos[1] > 0)
		{
			...
			data->point.x = self->tft.width() - pos[1];
			data->point.y = pos[0];
	...
	void setup() {
		...
		indev_drv.user_data = this;   //Pass pointer to current instance
